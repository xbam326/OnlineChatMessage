...
def make_chatroom():
  ...
  while True:
    ...
    print(f"roomname: {roomname}, operation: {operation}, state: {state}, username: {username}")
    connection.send(utils.build_message_for_tcrp(roomname, operation, utils.STATE_CODE.ACCEPTED, utils.STATUS_CODES["ACCEPTED"]))
    host = User(username, client_address)
    chatroom = ChatRoom(roomname, host)
    print(f"Chat room '{chatroom.roomname}' created by {chatroom.host.username} at {chatroom.host.address} with UUID {chatroom.host.uuid}")
    connection.send(utils.build_message_for_tcrp(roomname, operation, utils.STATE_CODE.CREATED, chatroom.host.uuid))
